...
VisualGraph::VisualGraph(float minY, float maxY, int numMarkersY, int sizeRingQueue, int numFunctions)  {
  ...
  offset = 0;
  lineColors.emplace_back(255, 0, 0);
  lineColors.emplace_back(0, 255, 0);
  lineColors.emplace_back(0, 0, 255);
  lineColors.emplace_back(0, 255, 255);
  lineColors.emplace_back(255, 255, 0);
}
...
void VisualGraph::display() {
  ...
  for(int n = 0; n < data.size(); n++){
    ...
    // Draw f(x)
    Brain.Screen.setPenColor(lineColors[n % lineColors.size()]);
